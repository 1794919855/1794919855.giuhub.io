...
var S = {
  init: function () {
      ...
      try {
          if (i!== -1) {
              S.UI.simulate(decodeURIComponent(action).substring(i + 3));
          } else {
              S.UI.simulate('好久不见 朋友|祝你|生日快乐|happy birthday|#countdown 3||');
          }
      } catch (error) {
          console.error('解码URL参数出现错误:', error);
          // 可以在这里添加一些错误处理逻辑，比如采用默认值等
      }
